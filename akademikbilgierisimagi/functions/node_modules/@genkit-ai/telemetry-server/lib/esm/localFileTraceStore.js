...
export class LocalFileTraceStore {
    ...
    constructor(options) {
        ...
        this.indexRoot = path.resolve(os.tmpdir(), `./genkit-trace-index-${Date.now()}-${Math.random()}`);
    ...
    async init() {
        ...
        console.log(`Indexed ${list.traces.length} traces in ${Date.now() - time}ms in ${this.indexRoot}`);
    }
    async load(id) {
